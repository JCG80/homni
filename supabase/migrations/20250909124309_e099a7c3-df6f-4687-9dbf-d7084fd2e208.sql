-- Phase 1B: Initialize user module access and enhance admin functionality
...
-- Function to initialize user module access
...
BEGIN
  ...
  FOREACH module_id IN ARRAY default_modules
  LOOP
    INSERT INTO user_modules (user_id, module_id, is_enabled)
    VALUES (target_user_id, module_id, true)
...
BEGIN
  ...
  -- Process each module
  FOREACH module_id IN ARRAY module_ids
  LOOP
    -- Update or insert access
    INSERT INTO user_modules (user_id, module_id, is_enabled)
    VALUES (target_user_id, module_id, enable_access)
    ON CONFLICT (user_id, module_id)
