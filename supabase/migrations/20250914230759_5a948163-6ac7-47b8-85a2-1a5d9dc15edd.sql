...
DROP POLICY IF EXISTS "module_metadata_admin_all" ON public.module_metadata;

-- Drop all conflicting functions with CASCADE to handle any remaining dependencies
...
DROP FUNCTION IF EXISTS public.grant_user_role(uuid, public.app_role, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.revoke_user_role(uuid, public.app_role, text) CASCADE;
...
-- Recreate dependent policies using the new function
