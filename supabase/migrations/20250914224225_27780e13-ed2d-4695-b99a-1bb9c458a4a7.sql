-- Fix existing trigger issue before implementing role system
-- migrations/20250115_fix_role_trigger.sql

-- Drop the problematic trigger that references non-existent role_enum
DROP TRIGGER IF EXISTS validate_user_role_consistency_trigger ON public.user_roles;
DROP FUNCTION IF EXISTS public.validate_user_role_consistency();

-- Now proceed with core role management tables
...
-- Add scope support to user_roles safely
DO $$
BEGIN
...
-- Admin action challenges (2FA/OTP)
