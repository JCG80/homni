-- Fix function parameter name conflict by dropping and recreating
DROP FUNCTION IF EXISTS public.get_user_role_level(uuid);

-- Phase 1: Standardize on 'user' role and add missing columns
-- Map any existing 'member' roles to 'user' before enum conversion

-- First, update any existing 'member' roles to 'user' in user_profiles
...
-- Convert roles.name to use app_role enum (if roles table exists)
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'roles' AND table_schema = 'public') THEN
    ALTER TABLE public.roles 
    ALTER COLUMN name TYPE public.app_role USING name::public.app_role;
  END IF;
END $$;

-- Convert sod_conflicts to use app_role enum (if table exists)
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'sod_conflicts' AND table_schema = 'public') THEN
    ALTER TABLE public.sod_conflicts 
    ALTER COLUMN role_a TYPE public.app_role USING role_a::public.app_role,
    ALTER COLUMN role_b TYPE public.app_role USING role_b::public.app_role;
  END IF;
END $$;
