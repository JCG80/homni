-- Phase 1B: Initialize user module access and enhance admin functionality (Fixed indexes)
...
CREATE INDEX IF NOT EXISTS idx_module_access_audit_user_id ON public.module_access_audit(user_id);
CREATE INDEX IF NOT EXISTS idx_module_access_audit_created_at ON public.module_access_audit(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_system_modules_category ON public.system_modules(category);
CREATE INDEX IF NOT EXISTS idx_user_modules_composite ON public.user_modules(user_id, module_id, is_enabled);