-- COMPREHENSIVE ROLE SYSTEM UPGRADE - CORRECTED VERSION
-- Handles existing function conflicts and implements complete role system
...
VALUES ('comprehensive_role_system_upgrade_v2', NOW(), true);
...
BEGIN
    ...
    -- Ensure columns are NOT NULL where appropriate
...
-- STEP 6: Enhanced role management functions (new signatures)
...
BEGIN
    ...
    -- Drop conflicting policies on payment_records
    ...
    -- Drop conflicting policies on admin_actions_log
...
-- STEP 11: Migrate existing role data safely
DO $$
DECLARE
...
BEGIN
    ...
    -- Log successful migration count
    INSERT INTO public._migration_log (migration_name, executed_at, success, error_message)
    VALUES ('role_migration_completed', NOW(), true, 'Migrated ' || migration_count || ' role records');
END $$;
...
-- STEP 13: Setup automated cleanup with pg_cron (safe upsert)
DO $$
BEGIN
    -- Only add cron job if it doesn't exist
...
-- STEP 14: Final validation and completion logging
INSERT INTO public._migration_log (migration_name, executed_at, success)
VALUES ('comprehensive_role_system_complete', NOW(), true);
