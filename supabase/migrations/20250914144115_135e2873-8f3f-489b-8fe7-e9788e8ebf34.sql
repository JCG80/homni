...
CREATE POLICY "admin_actions_log_admin_only" ON public.admin_actions_log
    ...
    USING ((auth.uid() IS NOT NULL) AND (auth.role() = 'authenticated'::text) AND (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master_admin'::app_role)))
    WITH CHECK ((auth.uid() IS NOT NULL) AND (auth.role() = 'authenticated'::text) AND (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'master_admin'::app_role)));
...
CREATE POLICY "admin_audit_log_master_admin_only" ON public.admin_audit_log
    FOR ALL TO authenticated
    USING ((auth.uid() IS NOT NULL) AND (auth.role() = 'authenticated'::text) AND has_role(auth.uid(), 'master_admin'::app_role))
    WITH CHECK ((auth.uid() IS NOT NULL) AND (auth.role() = 'authenticated'::text) AND has_role(auth.uid(), 'master_admin'::app_role));
...
CREATE POLICY "analytics_events_admin_full_access" ON public.analytics_events
    FOR ALL TO authenticated
...
CREATE POLICY "analytics_events_user_own_data" ON public.analytics_events
...
CREATE POLICY "smart_start_submissions_authenticated_access" ON public.smart_start_submissions
    FOR SELECT TO authenticated
...
-- 6. Ensure error_tracking has proper access control
...
-- 7. Fix leads access - ensure proper company isolation
...
COMMENT ON POLICY "admin_actions_log_admin_only" ON public.admin_actions_log 
    IS 'Restricts admin action logs to authenticated admin users only';

COMMENT ON POLICY "analytics_events_admin_full_access" ON public.analytics_events 
    IS 'Allows admins full access to analytics events';

COMMENT ON POLICY "analytics_events_user_own_data" ON public.analytics_events 
    IS 'Users can only view their own analytics events';

COMMENT ON POLICY "leads_authenticated_access" ON public.leads 
