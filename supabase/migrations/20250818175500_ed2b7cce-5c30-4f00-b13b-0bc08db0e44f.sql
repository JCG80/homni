-- ROLE GRANTS COMPLETE SYSTEM: Clean implementation with correct parameter order

-- 1) Create the role_grants table
...
-- 4) Add validation trigger (safer than CHECK constraints)
...
-- 5) Create role grant management functions with correct parameter order

-- has_role_grant: Check if user has a specific role grant
...
-- grant_role: Grant a role to a user (admin-only)
...
-- revoke_role: Revoke a role from a user (admin-only)
...
-- is_master_admin: Check if user is master admin (base role OR granted role)
...
-- get_user_effective_roles: Get all effective roles (base + granted)
