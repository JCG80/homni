-- Phase 1: Core Role Management System (Fixed)
-- migrations/20250115_advanced_roles_core_fixed.sql
...
-- Add scope_key column to existing user_roles if it doesn't exist
DO $$
BEGIN
...
-- Add a normalized scope column for primary key
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_roles' AND column_name = 'scope_normalized') THEN
    ALTER TABLE public.user_roles ADD COLUMN scope_normalized text NOT NULL DEFAULT '';
  END IF;
END $$;

-- Update scope_normalized based on scope_key
UPDATE public.user_roles SET scope_normalized = COALESCE(scope_key, '') WHERE scope_normalized = '';

-- Update primary key to include normalized scope
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE table_name = 'user_roles' AND constraint_name = 'user_roles_pkey_with_scope') THEN
    ...
    ALTER TABLE public.user_roles ADD CONSTRAINT user_roles_pkey_with_scope PRIMARY KEY (user_id, role, scope_normalized);
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.role_template_items (
  ...
  scope_normalized text NOT NULL DEFAULT '',
  expires_days int,
  UNIQUE (template_id, role, scope_normalized)
);
...
-- Triggers for maintaining scope_normalized
CREATE OR REPLACE FUNCTION public.sync_scope_normalized()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  NEW.scope_normalized = COALESCE(NEW.scope_key, '');
  RETURN NEW;
END;
$$;

CREATE TRIGGER sync_user_roles_scope_normalized
  BEFORE INSERT OR UPDATE ON public.user_roles
  FOR EACH ROW EXECUTE FUNCTION public.sync_scope_normalized();

CREATE TRIGGER sync_template_items_scope_normalized
  BEFORE INSERT OR UPDATE ON public.role_template_items
  FOR EACH ROW EXECUTE FUNCTION public.sync_scope_normalized();
